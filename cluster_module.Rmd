...
filenames <- c("3692_2020-04-15_tree-clusters-dm/3692_2020-04-15_thresholds.csv", 
               "4382_2020-06-04_tree-clusters-dm/4382_2020-06-04_thresholds.csv")
X <- 1
Y <- 3
```

This report looks at the data comparing from  `r filenames[1] %>% separateText(.,"/",2) %>% separateText(.,"_th",1)`, at time point `r X`, to `r filenames[2] %>% separateText(.,"/",2) %>% separateText(.,"_th",1)`, at time point `r Y`. Replace the corresponding text in `threshold_files_chunk` if using different data.
...
clustersX <- readData(filenames[1]) %>% as_tibble()
...
clustersY <- readData(filenames[2]) %>% as_tibble()
...
d$Place <- d$Isolates %>% 
  lapply(., function(x) as.character(x) %>% separateText(.,"/",1)) %>% unlist()
d <- d[,c(5,1:4)]
...
d[d == "TRUE"] <- vals[1]
d[d == "FALSE"] <- vals[2]
places <- d$Place %>% unique()
place_colors <- c(rep(c("lightpink","thistle","wheat"), ceiling(length(places)/3)))

# table with colorful cells indicating where isolates are found
d %>% set_colnames(c("Source", "Isolates", paste0("Found in time point ", X, " dataset"), 
  ...
  formatStyle(3, backgroundColor = styleEqual(vals, c("lightsteelblue", "gray"))) %>% 
  formatStyle(4, backgroundColor = styleEqual(vals, c("lightgreen", "gray"))) %>% 
  formatStyle(5, backgroundColor = styleEqual(vals, c("lightsteelblue", "gray"))) %>% 
  formatStyle(1, backgroundColor = styleEqual(unique(d$Place), place_colors[1:length(places)]))
```
...
```{r}
tp[1:50,c(1,3:9)] %>% # -"Numeric height", 
...
# sizes.
increment_size <- 50
tp_subset <- tp %>% filter(h %in% seq(from = 1, to = length(num_h_vals), by = increment_size))
