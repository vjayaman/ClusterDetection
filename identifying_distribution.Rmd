...
```{r}
gofTest(x_shifted, test = "cvm", distribution = "lnorm")
```
...
# here, we use the dlnorm function, since we're looking at the lognormal
# there are other functions for each type of traditional distribution
null_probs <- dlnorm(1:length(x_shifted), 
                     meanlog = best_fit$estimate[1], 
                     sdlog = best_fit$estimate[2])
...
Alternatively, we could look at the data as binary, either a cluster increased in size, or it did not. This might be useful when we have a very sparse set of changes in size, as in the example below. 
...
Note that `r sparseCheck(hist2)` of the histogram bins are empty. When this number is high, we have very sparse dataset, so finding the probability of a cluster's size increasing is tricker.
...
Consider: draw type 1 - the cluster grows in size, and draw type 2 - the cluster stays the same size or shrinks.
